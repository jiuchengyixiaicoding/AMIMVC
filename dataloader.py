...
class Fashion(Dataset):
    def __init__(self, path):
        self.Y = scipy.io.loadmat(path)['Y'].astype(np.int32).reshape(10000,)
        self.V1 = scipy.io.loadmat(path)['X1'].reshape(10000,-1).astype(np.float32)
        self.V2 = scipy.io.loadmat(path)['X2'].reshape(10000,-1).astype(np.float32)
        print(scipy.io.loadmat(path).keys())
        sys.exit()
    def __len__(self):
        return self.V1.shape[0]
    ...
    def __getitem__(self, idx):
        return [torch.from_numpy(self.V1[idx]), torch.from_numpy(
           self.V2[idx])], torch.from_numpy(np.array(self.Y[idx])), torch.from_numpy(np.array(idx)).long()
...
class HandWritten(Dataset):
    def __init__(self, path):
        ...
        X = mat['X'][0]
        data1 = X[0].astype(np.float32)
        data2 = X[1].astype(np.float32)
        labels = mat['Y']
        self.V1 = data1
        self.V2 = data2
        self.Y = labels
...
def load_data(dataset):
    if dataset == "BDGP":
        ...
        dataset = HandWritten(('./data/' + dataset + '.mat'))
        dims = [240, 76]
        view = 2
        class_num = 10
        data_size = 2000
    elif dataset == "Fashion":
        dataset = Fashion(('./data/' + dataset + '.mat'))
        dims = [784, 784]
        view = 2
        class_num = 10
        data_size = 10000
    elif dataset == "caltech101":
        dataset = Caltech101(('./data/' + dataset + '.mat'))
        dims = [4096, 4096]
        view = 2
        class_num = 101
        data_size = 8677
